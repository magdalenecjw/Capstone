...
plot_data <- data.frame(
  Date = as_datetime(temporal_density$x),
...
temporal_margin <- ggplot(plot_data, aes(x = Date, y = Density)) +
  ...
  theme_minimal() +
  labs(title = "Temporal margin\n Pick-up Events across Singapore",
       x = "Date",
       y = "Events per square kilometre per second)") +
...
ggsave('pickup_temp_overall.png', temporal_margin, width = 12, height = 5.5)
```
...
```{r}
#| eval: false
# Define dates (assuming dates is a vector of date values)
dates <- as.numeric(seq(min(as_datetime(pickup_scaled$marks)), 
                        max(as_datetime(pickup_scaled$marks)), 
                        by = "6 hours"))
...
temporal_margin <- ggplot(plot_data, aes(x = Date, y = Density)) +
  ...
  scale_x_datetime(date_labels = "%b-%d", date_breaks = "1 day", minor_breaks = NULL) +
  theme_minimal() +
  labs(title = "Temporal margin\n Drop-off Events across Singapore",
       x = "Date",
       y = "Events per square kilometre per second)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_line(color = "gray80"),
...
ggsave('dropoff_temp_overall.png', temporal_margin, width = 12, height = 5.5)
```
