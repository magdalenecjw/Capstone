...
# KDE Analysis using using differnet bandwidth methods

Computing a kernel density using fixed bandwidth can be done using the `density()` function of spatstat:
...
However, the fixed bandwidth method is very sensitive to highly skew distribution of spatial point patterns over geographical units for example urban versus rural. One way to overcome this problem is by using adaptive bandwidth - in spatstat package, this can be derived using `density.adaptive()` function.
...
# Fixed Bandwidth - LSCV
pickup_kde <- density(pickup_owin, sigma=bw.diggle, edge=TRUE) 
...
png("pickup_adapt.png", width=1000, height=700)
par(mfrow=c(2,2))
plot(pickup_kde, main="Least Squares Cross Validation")
plot(pickup_ppl, main="Likelihood Cross Validation")
plot(pickup_adapt, main="Adaptive Bandwidth Selection")

png("dropoff_adapt.png", width=1000, height=700)
par(mfrow=c(2,2))
plot(dropoff_kde, main="Least Squares Cross Validation")
plot(dropoff_ppl, main="Likelihood Cross Validation")
plot(dropoff_adapt, main="Adaptive Bandwidth Selection")
```
